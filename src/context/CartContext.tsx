...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  switch (action.type) {
    case 'ADD_TO_CART': {
      ...
      // Un producto es único por id + keyType
      const existingItem = state.items.find(
        item => item.product.id === product.id && item.product.keyType === product.keyType
      );

      let newItems: CartItem[];
      if (existingItem) {
        newItems = state.items.map(item =>
          item.product.id === product.id && item.product.keyType === product.keyType
        ...
        newItems = [
          ...state.items,
          { id: `${product.id}-${product.keyType}-${Date.now()}`, product, quantity }
        ];
      }
