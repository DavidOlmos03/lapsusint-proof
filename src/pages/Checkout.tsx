...
function getPriceByType(product: any, type: string) {
  ...
  let price = 0;
  switch (type) {
    case 'one_week': price = product.price_one_week; break;
    case 'one_month': price = product.price_one_month; break;
    case 'three_months': price = product.price_three_months; break;
    case 'lifetime': price = product.price_lifetime; break;
    default: price = product.price_one_month || product.price; break;
  }
    ...
    try {
      ...
      for (const item of cartState.items) {
        ...
        const orderResponse = await fetch(
          `${process.env.REACT_APP_API_URL || "http://127.0.0.1:8000"}/orders`,
          {
            method: 'POST',
            headers: {
        ...
        const completeResponse = await fetch(
          `${process.env.REACT_APP_API_URL || "http://127.0.0.1:8000"}/orders/${order.order_id}/complete`,
          {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
      ...
      else if (error && error.detail) errorMessage = error.detail;
              ...
              {cartState.items.map(item => {
                ...
                return (
                  <ProductItem key={item.id}>
                    <ProductImage src={getImageUrl(item.product.image_url)} alt={item.product.product_name} />
                    <ProductDetails>
