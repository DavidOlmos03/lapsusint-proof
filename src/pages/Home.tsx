...
const Home: React.FC = () => {
  ...
  const confirmDeleteProduct = async () => {
    ...
    try {
      // Usar license_id o id según corresponda
      const deleteId = productToDelete.license_id || productToDelete.id || productToDelete.product_id;
      await deleteProduct(token, deleteId);
      setProducts(products.filter((p) => (p.license_id || p.id || p.product_id) !== deleteId));
  ...
  const handleSubmit = async (data: any) => {
    try {
      if (modalMode === 'add') {
        ...
        const updatedProduct = await updateProduct(token, editProduct.id, data);
